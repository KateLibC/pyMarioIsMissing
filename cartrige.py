...
class ROMHeader(object):
    ...
    def parse_header(self, rom_byte_array, start_address):
        ...
        self.name      = self.name.decode() # Added by Cariad
    ...
    def dump(self):
        ...
        print("LICENCE CODE:       \t0x" + hex(self.licence_code))
        print("VERSION:            \t"   + hex(self.version) + " (1." + str(self.version) +  ")")
        print("CHECKSUM COMPLEMENT:\t0x" + hex(self.checksum_complement) + " (NOT CHECKSUM)")
        print("CHECKSUM:           \t0x" + hex(self.checksum) + " (SUM OF BYTES AND 0xFFFF)")
        print("CHECKSUM STATUS:    \t"   + str(hex(self.checksum)==self.expected_checksum))
        print("")
        print("NATIVE COP INTERRUPT ADDR:  \t0x" + hex(self.native_cop_int_addr))
        print("NATIVE BRK INTERRUPT ADDR:  \t0x" + hex(self.native_brk_int_addr))
        print("NATIVE ABORT INTERRUPT ADDR:\t0x" + hex(self.native_abort_int_addr))
        print("NATIVE RESET INTERRUPT ADDR:\t0x" + hex(self.native_reset_int_addr))
        print("NATIVE IRQ INTERRUPT ADDR:  \t0x" + hex(self.native_irq_int_addr))
        print("COP INTERRUPT ADDR:         \t0x" + hex(self.cop_int_addr))
        print("ABORT INTERRUPT ADDR:       \t0x" + hex(self.abort_int_addr))
        print("NMI INTERRUPT ADDR:         \t0x" + hex(self.nmi_int_addr))
        print("RESET INTERRUPT ADDR:       \t0x" + hex(self.reset_int_addr))
        print("IRQ INTERRUPT ADDR:         \t0x" + hex(self.irq_int_addr))
